...
class GCSLfsTransferAgent:
    ...
    def handle_request(self):
        ...
        sys.stderr.write(f"Read payload: {payload}\n")
        sys.stderr.flush()
        ...
        if action == "init":
            sys.stderr.write("Detected init event\n")
            sys.stderr.flush()
            self.handle_init(payload)
        elif action == "terminate":
            self.handle_terminate(payload)
        elif action == "upload":
            object_name = oid  # Use the object ID as the file name in GCS
            self.upload(bucket, object_name, path)
        elif action == "download":
            object_name = oid
            self.download(bucket, object_name, path)
        else:
